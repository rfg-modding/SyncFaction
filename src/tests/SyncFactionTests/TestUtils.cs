...
public static class TestUtils
{
    ...
    /// </summary>
    public static IEnumerable<TestCaseData> AllArchiveFiles() => AllVppFiles().Concat(AllExtractedStr2Once);
    ...
    //.Skip(10)
    public static IEnumerable<TestCaseData> AllFiles() => AllVppFilesOnce.Concat(AllExtractedOnce);

    public static IEnumerable<TestCaseData> AllModInfos() => AllModInfosOnce;
    ...
    {
        UnpackDir.Create();
        return UnpackDir.EnumerateFiles("*", SearchOption.AllDirectories).OrderBy(x => x.FullName).Select(x => new TestCaseData(x).SetArgDisplayNames(x.FullName.Substring(UnpackDir.FullName.Length + 1))).ToList();
    }

    private static List<TestCaseData> InitAllExtractedStr2Once()
    {
        UnpackDir.Create();
        return UnpackDir.EnumerateFiles("*.str2_pc", SearchOption.AllDirectories).OrderBy(x => x.FullName).Select(x => new TestCaseData(x).SetArgDisplayNames(x.FullName.Substring(UnpackDir.FullName.Length + 1))).ToList();
    }
    ...
    private static readonly List<TestCaseData> AllExtractedOnce = InitAllExtractedOnce();
    ...
    private static readonly List<TestCaseData> AllExtractedStr2Once = InitAllExtractedStr2Once();
    private static readonly List<TestCaseData> AllModInfosOnce = InitAllModInfoOnce();
