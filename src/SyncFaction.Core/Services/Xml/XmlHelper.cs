...
public static class XmlHelper
{
    ...
    internal static IXmlNodeMatcher BuildSubnodeValueMatcher(string criteria, XmlNode editChild)
    {
        ...
        var xPaths = criteria.Split(',').Select(XmlHelper.FormatXPath).ToList();
        var firstXpath = xPaths.First();
        var value = XmlHelper.GetSubnodeByXPath(editChild, firstXpath)?.InnerText;
        if (string.IsNullOrWhiteSpace(value))
        {
        ...
        {
            var result = XmlHelper.GetSubnodeByXPath(editChild, x)?.InnerText ?? string.Empty;
    ...
    public static XmlNode AppendNewChild(string name, XmlNode target)
    {
