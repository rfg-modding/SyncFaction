...
public class Tools
{
    ...
    public Tools(IHttpClientFactory httpClientFactory, ILogger<Tools> log, IHostApplicationLifetime lifetime=null)
    {
    ...
    public async Task ApplyPlaylist(DirectoryInfo dataDir, DirectoryInfo bakDir, DirectoryInfo playlistDir, CancellationToken token, Action<string> callback=null)
    {
        var remainingFiles = new HashSet<string>(knownFiles.Keys);
        ...
        foreach (var x in playlistDir.EnumerateFiles())
        {
            var item_name = Path.GetFileNameWithoutExtension(x.Name);
            var fileName = Path.GetFileNameWithoutExtension(x.Name) + ".vpp_pc";
            ...
            using var srcStream = new FileInfo(Path.Combine(bakDir.FullName, fileName)).OpenRead();
            using var patchStream = x.OpenRead();
            using var dstStream = dstFile.Open(FileMode.Create, FileAccess.ReadWrite);
    ...
    public async Task<DirectoryInfo> DownloadPlaylist(DirectoryInfo baseDir, Link playlist, CancellationToken token)
    {
        var playlistDir = new DirectoryInfo(Path.Combine(baseDir.FullName, playlist.Name));
        ...
        var links = await GetPlaylist(playlist, token);

        var client = httpClientFactory.CreateClient();
        foreach (var fileLink in links)
        {
            var destination = new FileInfo(Path.Combine(playlistDir.FullName, fileLink.Name));
            using var dstStream = destination.OpenWrite();
            var response = await client.GetAsync(fileLink.Url, token);
            response.EnsureSuccessStatusCode();
            var s = await response.Content.ReadAsStreamAsync();
            await s.CopyToAsync(dstStream, 81920, token);

            log.LogDebug("Downloaded {url} as {file}", fileLink.Url, destination.FullName);
        }
    ...
    {
        var appDir = new DirectoryInfo(Path.Combine(dataDir.FullName, appDirName));
        var bakDir = new DirectoryInfo(Path.Combine(appDir.FullName, bakDirName));
        if (!bakDir.Exists)
        {
            bakDir.Create();
        }
        ...
        var filesToCopy = filesToBackUp.Intersect(knownFiles.Keys).Except(existingMapFiles);
        ...
        if (filesToCopy.Any())
        {
            ...
            foreach (var x in filesToCopy)
            {
                ...
                if (!CheckHash(source, knownFiles[x]))
                {
    ...
    public async Task<List<Link>> GetIndexLinks(string url, CancellationToken token)
    {
        ...
        var document = await parser.ParseDocumentAsync(contentStream, token);
        return document.QuerySelectorAll("a")
            .Where(x => x.Attributes["href"] is not null)
            .Where(x => !x.Attributes["href"]!.Value.StartsWith("/"))
            .Select(x => new Link(x, url))
            .ToList();
    }

    public async Task<string> GetNews(List<Link> links, CancellationToken token)
    {
        var link = links.SingleOrDefault(x => x.Name.ToLowerInvariant() == newsFile);
        if (link is null)
        {
            return "> *No news from server*";
        }

        var response = await httpClientFactory.CreateClient().GetAsync(link.Url, token);
        return await response.EnsureSuccessStatusCode().Content.ReadAsStringAsync();
    }

    public async Task<string> GetReadme(Link playlistLink, CancellationToken token)
    {
        var readme = playlistLink.Url.TrimEnd('/') + "/" + readmeFile;
        var response = await httpClientFactory.CreateClient().GetAsync(readme, token);
        if (response.StatusCode == HttpStatusCode.NotFound)
        {
            return "No readme.md provided";
        }
        return await response.EnsureSuccessStatusCode().Content.ReadAsStringAsync();
    }

    public List<Link> GetPlaylists(List<Link> links)
    {
        return links.Where(x => x.Name.EndsWith("/")).OrderBy(x => x.Name).ToList();
    }

    public async Task<List<FileLink>> GetPlaylist(Link link, CancellationToken token)
    {
        var httpClient = httpClientFactory.CreateClient();
        var response = await httpClient.GetAsync(link.Url, token);
        response.EnsureSuccessStatusCode();
        var contentStream = await response.Content.ReadAsStreamAsync();
        var parser = new AngleSharp.Html.Parser.HtmlParser();
        var document = await parser.ParseDocumentAsync(contentStream, token);
        return document.QuerySelectorAll("a")
            .Where(x => x.Attributes["href"] is not null)
            .Where(x => !x.Attributes["href"]!.Value.StartsWith("/"))
            .Where(x => IsKnown(x.TextContent))
            .Select(x => new FileLink(x, link.Url))
            .ToList();
    }
    ...
    public bool IsKnown(string value)
    {
        ...
        return knownFiles.ContainsKey(name);
    }

    public readonly ImmutableDictionary<string, string> knownFiles = new Dictionary<string, string>()
    {
...
{"zonescript_terr01","04b9db2005d1d621d4acda7ba7438fde39864c7986d90c063acd8f2487343c79"},
    ...
    public const string newsFile = "news.md";
    public const string readmeFile = "readme.md";
    public const string defaultUrl = "http://localhost:8000";
    public const string defaultGamePath = @"C:\Program Files (x86)\Steam\steamapps\common\Red Faction Guerrilla Re-MARS-tered";
